...
- Update the clarus-pod.yaml. This time we will increase the `resources.limits` more than maximum limit. 
...
```bash
Error from server (Forbidden): error when creating "clarus-pod.yaml": pods "default-cpu" is forbidden: maximum cpu usage per Container is 1500m, but limit is 2
```
...
- Update the clarus-pod.yaml. This time we will increase `resources.requests` more than `default limit`.
